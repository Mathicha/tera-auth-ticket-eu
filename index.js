...
class auth {
   ...
   */
  constructor(logger = console, logLevel = 'log') {
    this.logger = logger;
      ...
      this.getAccounts({ bearer }, (err, accounts) => {
        ...
        if (!accounts.length) return cb(new Error('No TERA account on this Gameforge account.'));
          ...
          if (accounts.length > 1)
            this.logger[this.logLevel](`Multiples TERA accounts are linked to this Gameforge account, I'll use TERA account: '${accountId}'.`);
          ...
          else this.logger[this.logLevel](`TERA account '${submittedAccountId}' don't exist on this Gameforge account, I'll use TERA account: '${accountId}'.`);
        }
